...
class SubscriptionRepository extends BaseRepository{
    ...
    public function store(array $input){
        $subscription_plan = SubscriptionPlan::find($input['subacription_plan_id']);
        ...
        $subscription_plan_duration_unit = $subscription_plan->duration_unit;

        if(!$tenant){
            return response()->json([
                'status'  => 'error',
                'message' => __('locale.tenant_not_found'),
            ]);
        }

        if($tenant->activeSubscription()){
        //    cancel the subscription or warn the tenant  figure way to handle this
        // TODO
        }
       

        $subscription_plan_id = Subscription::where('tenant_id', $input['tenant_id'])->latest();
        if($tenant->subscription() && $input['subscription_plan_id'] === $subscription_plan_id){
            //check if the subscription_plan_id = the last subscription_plan_id   
                ...
                //set the subscription_end_at accordingly to set subscription -duration and duration_unit
        }
        ...
        //transaction check if it goes through incase it doesnt it reverse the process 
    ...
    //handle free trials if they exist in the subscription plan


    //make payment
    public function payPayment(SubscriptionPlan $plan, Subscription $subscription, array $input): JsonResponse
    {
            ...
            switch ($paymentMethod->type) {
                ...
                case PaymentMethods::TYPE_MYFATOORAH:
                    ...
                    $invoiceItems[] = [
                        ...
                        'UnitPrice' => $plan->price, //Price per item
                    ];
